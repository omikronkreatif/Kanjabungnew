...
class SimPinMember(models.Model):
    ...
    kabkota_id_clone = fields.Char( string='Kab / Kota')
    provinsi_id = fields.Many2one('wilayah.provinsi', string='Provinsi')
    provinsi_id_clone = fields.Char(string='Provinsi')
    ...
    wilayah_id_clone = fields.Char('Wilayah')
    ko = fields.Char('KO')
    ka = fields.Char('KA')
    jns_keanggotan = fields.Selection([
        ('alb', 'ALB'),
        ('a', 'A'),
    ], string='Jenis Keanggotaan')
    sk = fields.Char('SK')
    ...
    def _onchange_wilayah_id(self):
        if self.wilayah_id:
            self.wilayah_id_clone = self.wilayah_id.wilayah_id
        else:
...
#        raise UserError(_('total_pinjaman %s \n %s')%(total_pinjaman,self.pinjaman_total,))
    ...
    def action_approve(self):
        ...
        if self.is_sukarela:
            simp_pokok = simp_wajib = False
            simp_pokok = self.env['simpin_syariah.rekening'].sudo().create({
                                'name': 'simpanan pokok',
                                'member_id': self.id,
                                'partner_id': partner_id.id,
                                'akad_id': akad_syariah_pokok.id,
                                'product_id': s_pokok.id,
                                'account_analytic_id': account_analytic_id.id,
                                })
            simp_pokok.action_approve()

            simp_wajib = self.env['simpin_syariah.rekening'].sudo().create({
                                'name': 'simpanan wajib',
                                'member_id': self.id,
                                'partner_id': partner_id.id,
                                'akad_id': akad_syariah_wajib.id,
                                'product_id': s_wajib.id,
                                'account_analytic_id': account_analytic_id.id,
                                'state': 'active',
                                })
            simp_wajib.action_approve()
                
            simp_sukarela = self.env['simpin_syariah.rekening'].sudo().create({
                                'name': '/',
                                'member_id': self.id,
                                'partner_id': partner_id.id,
                                'akad_id': akad_syariah_sukarela.id,
                                'product_id': s_sukarela.id,
                                'account_analytic_id': account_analytic_id.id,
                                'state': 'active',
                                })
            simp_sukarela.action_approve()
                
            self.write({'state': 'approve',
                        'partner_id': partner_id.id,
                        'simpanan_pokok': simp_pokok.id,
                        'simpanan_wajib': simp_wajib.id,
                        'simpanan_sukarela': simp_sukarela.id,
                        })
        else:
            simp_pokok = simp_wajib = False
            simp_pokok = self.env['simpin_syariah.rekening'].sudo().create({
