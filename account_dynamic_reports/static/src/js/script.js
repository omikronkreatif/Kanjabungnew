		...
		plot_data : function(initial_render = true){
			...
			self._rpc({
				...
				var formatOptions = {
					...
					_.each(self.account_data, function (k, v){
						...
						k.balance_init = self.formatWithSign(k.balance_init, formatOptions, k.balance_init < 0 ? '-' : '');
						k.balance_prev = self.formatWithSign(k.balance_prev, formatOptions, k.balance_prev < 0 ? '-' : '');
						k.balance_budget_month = self.formatWithSign(k.balance_budget_month, formatOptions, k.balance_budget_month < 0 ? '-' : '');
						k.balance_budget_year = self.formatWithSign(k.balance_budget_year, formatOptions, k.balance_budget_year < 0 ? '-' : '');
					});
					if(initial_render){
						...
						self.$el.find('.warehouse-multiple').select2({
							placeholder:'Select Warehouse...',
						});

						self.$el.find('.department-multiple').select2({
			...
			view_gl : function(event){
				...
				var domains = {
					...
					var context = {};
					domains.target_moves = self.filter_data.target_moves;
					if ($("#date_from").val()){
					...
					if(!domains.date_from && !domains.date_to && !domains.date_range){domains.date_from = self.filter_data.date_from;
						...
						var warehouse_ids = [];
						var warehouse_list = $(".warehouse-multiple").select2('data')
						for (var i=0; i < warehouse_list.length; i++){
							warehouse_ids.push(parseInt(warehouse_list[i].id))
						}
						domains.warehouse_ids = warehouse_ids

						var department_ids = [];
						var department_list = $(".department-multiple").select2('data')
						for (var i=0; i < department_list.length; i++){
							department_ids.push(parseInt(department_list[i].id))
						}
						domains.department_ids = department_ids
					...
					update_with_filter : function(event){
						...
						output.warehouse_ids = warehouse_ids
						...
						output.department_ids = department_ids
...
var DynamicPaMain = AbstractAction.extend({
	...
	// menambahkan warehouse dan department
