...
class InsFinancialReportXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, data):
        ...
        if data['form']['debit_credit'] == 1:
            ...
            self.sheet.set_column(4, 3, 15)

            self.sheet.write_string(self.row_pos, 0, _('Name'),
                                    self.format_header)
            self.sheet.write_string(self.row_pos, 1, _('Initial Balance'),
                                    self.format_header)
            ...
            for a in data['report_lines']:
                ...
                self.sheet.write_string(self.row_pos, 0, '   ' * len(a.get('list_len', [])) + a.get('name'),
                                        tmp_style_str)
                self.sheet.write_number(self.row_pos, 1, float(a.get('balance_init')), tmp_style_num)
        ...
        if data['form']['debit_credit'] != 1:
            ...
            if data['form']['enable_filter']:
                self.sheet.write_string(self.row_pos, 1, data['form']['label_filter'],
                                        self.format_header)
                self.sheet.write_string(self.row_pos, 2, _('Initial Balance'),
                                        self.format_header)
            ...
            for a in data['report_lines']:
                ...
                if data['form']['enable_filter']:
                    ...
                    self.sheet.write_number(self.row_pos, 2, float(a.get('balance_init')), tmp_style_num)
