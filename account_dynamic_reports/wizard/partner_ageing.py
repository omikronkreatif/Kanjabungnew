...
#    @api.multi
    ...
    def get_filters(self, default_filters={}):
        ...
        warehouses = [(w.id,w.name) for w in self.env['stock.warehouse'].search([('company_id','=',company_id.id)])]
        departments = [(d.id,d.name) for d in self.env['hr.department'].search([])]
        filter_dict = {
    ...
    def process_detailed_data(self, offset=0, partner=0, fetch_range=FETCH_RANGE):
        ...
        if partner:
            ...
            params = [ type, 
                    ...
                    as_on_date, 
                    company_id.id,
                    account_ids,tuple(self.account_ids.ids) or (None,),                    
                    warehouse_ids,tuple(self.warehouse_ids.ids) or (None,),
                    department_ids,tuple(self.department_ids.ids) or (None,) ,
                    ]
    ...
    def process_data(self):
        ...
        for partner in partner_ids:
            ...
            params = [type, partner.id, as_on_date, company_id.id,account_ids,tuple(self.account_ids.ids) or (None,),
                       warehouse_ids,tuple(self.warehouse_ids.ids) or (None,),
                    department_ids,tuple(self.department_ids.ids) or (None,) ,
                    ]
            self.env.cr.execute(sql,tuple(params))
