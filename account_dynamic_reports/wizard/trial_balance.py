...
#    @api.multi
    ...
    def process_data(self, data):
        if data:
            ...
            # if data.get('warehouse_ids', []):
            #     WHERE += ' AND l.warehouse_id IN %s' % str(tuple(data.get('warehouse_ids')) + tuple([0]))
            
            # if data.get('department_ids', []):
            #     print ('department xxxxxxxxxxxxxxxx')
    ...
    def get_filters(self, default_filters={}):
        ...
        departments = self.department_ids if self.department_ids else self.env['hr.department'].search([('company_id','=',company_id.id)])
        ...
        filter_dict = {
            ...
            'department_list': [(d.id, d.name) for d in departments],
            'company_name': self.company_id and self.company_id.name,
        }
        print (filter_dict['warehouse_ids'])
