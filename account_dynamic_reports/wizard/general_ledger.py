...
class InsGeneralLedger(models.TransientModel):
    ...
    target_moves = fields.Selection(
        [('all', 'All entries'),
         ('posted', 'Posted Only')], string='Target Moves',
...
#    @api.multi
    def write(self, vals):
        ...
        if vals.get('department_ids'):
            vals.update({'department_ids': [(4, j) for j in vals.get('department_ids')]})
        if vals.get('department_ids') == []:
    ...
    def process_filters(self):
        ...
        if data.get('warehouse_ids', []):
           filters['warehouses'] = self.env['stock.warehouse'].browse(data.get('warehouse_ids', [])).mapped('name')
        else:
           filters['warehouses'] = ['All']
        if data.get('department_ids', []):
           filters['departments'] = self.env['hr.department'].browse(data.get('department_ids', [])).mapped('name')
        else:
           filters['departments'] = ['All']
        ...
        filters['warehouse_list'] = data.get('warehouse_list')
        filters['department_list'] = data.get('department_list')
    ...
    def build_where_clause(self, data=False):
        ...
        if data:
            ...
            # if data.get('warehouse_ids',[]):
            #    WHERE += ' AND wh.id IN %s' % str(tuple(data.get('warehouse_ids')) + tuple([0]))

            # if data.get('department_ids',[]):
            #    WHERE += ' AND l.department_id IN %s' % str(tuple(data.get('department_ids')) + tuple([0]))
            ...
            if data.get('target_moves') == 'posted':
    ...
    def build_detailed_move_lines(self, offset=0, account=0, fetch_range=FETCH_RANGE):
        ...
        full_sql += 'runing balance\n' + sql + '\n\n'
#        raise UserError(_('sql %s')%(full_sql,))
        sql = ('''
            SELECT COUNT(*)
            FROM account_move_line l
                JOIN account_move m ON (l.move_id=m.id)
        ...
        for account in account_ids:
            ...
            if data.get('initial_balance'):
                sql = ('''
                    ...
                    WHERE %s
                ''') % WHERE_INIT
#                raise UserError(_('init_sql %s')%(sql,))
                cr.execute(sql)
                for row in cr.dictfetchall():
                    row['move_name'] = 'Initial Balance'
...
#            raise UserError(_('current_sql %s')%(sql,))
            cr.execute(sql)
...
#                                    ('supplier','=',True),
        ...
        warehouses = self.warehouse_ids if self.warehouse_ids else self.env['stock.warehouse'].search(company_domain)
        departments = self.department_ids if self.department_ids else self.env['hr.department'].search(company_domain)
        ...
        filter_dict = {
            ...
            'warehouse_list': [(wh.id, wh.name) for wh in warehouses],
            'department_list': [(dep.id, dep.name) for dep in departments],
